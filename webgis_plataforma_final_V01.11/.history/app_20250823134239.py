...
def get_plan_prices():
    """Retorna os preços dos planos calculados dinamicamente"""
    try:
        ...
        for plan_name, config in plans.items():
            ...
            result = calculate_pricing_internal(config)
            if result:
...
def calculate_pricing_internal(data):
    """Função interna para calcular preços (sem request)"""
    try:
        ...
        for config in configs:
            if config.category == 'projects':
                cost = calculate_item_cost(data.get('projects', 0), config)
                breakdown['projects'] = {
                    'cost': cost,
                    'weight': config.weight_factor,
                    'weighted_cost': cost * config.weight_factor
                }
                total_cost += cost * config.weight_factor
                
            elif config.category == 'layers':
                cost = calculate_item_cost(data.get('layers', 0), config)
                breakdown['layers'] = {
                    'cost': cost,
                    'weight': config.weight_factor,
                    'weighted_cost': cost * config.weight_factor
                ...
                cost = calculate_item_cost(data.get('public_maps', 0), config)
                ...
                cost = calculate_item_cost(data.get('page_views', 0), config)
                ...
                cost = calculate_item_cost(data.get('employees', 0), config)
                breakdown['employees'] = {
                    'cost': cost,
                    'weight': config.weight_factor,
                    'weighted_cost': cost * config.weight_factor
                ...
                cost = calculate_item_cost(data.get('server_usage', 0), config)
        ...
        print(f"Erro no cálculo interno: {e}")
        return None

def calculate_item_cost(quantity, config):
    """Calcula o custo de um item baseado na configuração de tier"""
    try:
        if quantity <= 0:
            return 0
        ...
        for i, limit in enumerate(tier_limits):
            ...
            if i == 0:
                ...
                cost += tier_quantity * tier_prices[i]
        ...
        if remaining_quantity > 0 and tier_prices:
            cost += remaining_quantity * tier_prices[-1]
        
        return cost
        
    except Exception as e:
        print(f"Erro ao calcular custo do item: {e}")
        return 0
