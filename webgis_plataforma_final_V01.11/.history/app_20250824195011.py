...
class ContactMessage(db.Model):
    ...
    funcionarios = db.Column(db.String(50), nullable=True)
    urgencia = db.Column(db.String(100), nullable=True)
    mensagem = db.Column(db.Text, nullable=True)
    newsletter = db.Column(db.Boolean, default=False)
    plano_escolhido = db.Column(db.Text, nullable=False)  # JSON com dados do plano
    data_envio = db.Column(db.String(20), nullable=False)
    hora_envio = db.Column(db.String(20), nullable=False)
    timestamp = db.Column(db.DateTime, default=datetime.utcnow)
    status = db.Column(db.String(20), default='novo')  # novo, lido, respondido, arquivado
...
class GeographicReport(db.Model):
    """Modelo para Relatórios Geográficos"""
    id = db.Column(db.Integer, primary_key=True)
    title = db.Column(db.String(200), nullable=False)
    description = db.Column(db.Text)
    user_id = db.Column(db.Integer, db.ForeignKey('user.id'), nullable=False)
    company_id = db.Column(db.Integer, db.ForeignKey('company.id'), nullable=False)
    config = db.Column(db.JSON)  # Configurações do relatório (camadas, paradas, design)
    ...
    # Relacionamentos
    user = db.relationship('User', backref='geographic_reports')
    company = db.relationship('Company', backref='geographic_reports')
    
    def __repr__(self):
        return f'<GeographicReport {self.title}>'
...
@app.route('/precos')
def precos():
    """Página dedicada de preços e planos"""
    return render_template('precos.html')

@app.route('/contato')
def contato():
    """Página de contato para escolha de planos"""
...
def company_portal(company_slug):
    ...
    # Se o usuário está logado e pertence a esta empresa, mostrar o WebGIS
    if current_user.is_authenticated:
        if current_user.company and current_user.company.slug == company_slug:
            return render_template('portal.html', company=company)
        else:
            # Usuário logado mas não pertence a esta empresa
            flash('Você não tem acesso a este portal.', 'error')
            return redirect(url_for('home'))
    
    # Usuário não logado, mostrar página pública da empresa
...
def calculate_pricing():
    """API para calcular preços com base nos parâmetros fornecidos"""
    try:
        ...
        # Usar a mesma função de cálculo detalhado dos planos
        result = calculate_pricing_internal_detailed(data)
        ...
        print(f"Erro na API de cálculo: {e}")
        import traceback
        traceback.print_exc()
        return jsonify({'error': 'Erro interno do servidor'}), 500

@app.route('/api/pricing/debug', methods=['GET'])
def debug_pricing():
    """Rota de debug para verificar configuração dos tiers"""
    try:
        configs = PricingConfig.query.filter_by(is_active=True).all()
        
        debug_info = {}
        for config in configs:
            debug_info[config.name] = {
            ...
            for i in range(1, 7):
                tier_price = getattr(config, f'tier_{i}_price', 0)
                tier_limit = getattr(config, f'tier_{i}_limit', 0)
                if tier_price > 0 and tier_limit > 0:
                    debug_info[config.name]['tiers'][f'tier_{i}'] = {
                        'price': tier_price,
                        'limit': tier_limit,
                        'discount_percentage': ((config.base_price - tier_price) / config.base_price * 100) if config.base_price > 0 else 0
                    }
...
def api_contact():
    ...
    try:
        print("DEBUG: Recebendo requisição POST para /api/contact")
        data = request.get_json()
        print(f"DEBUG: Dados recebidos: {data}")
        
        if not data:
            print("DEBUG: Erro: Dados não fornecidos")
            return jsonify({
                'success': False,
        ...
        print(f"DEBUG: Validando campos obrigatórios: {required_fields}")
        for field in required_fields:
            if not data.get(field):
                print(f"DEBUG: Erro: Campo obrigatório não fornecido: {field}")
                return jsonify({
                    'success': False,
                    'error': f'Campo obrigatório não fornecido: {field}'
                }), 400
        
        # Verificar se o plano foi fornecido (pode vir como 'plano' ou 'plano_escolhido')
        plano_data = data.get('plano') or data.get('plano_escolhido')
        if not plano_data:
            print("DEBUG: Erro: Dados do plano não fornecidos")
            return jsonify({
                'success': False,
                'error': 'Dados do plano não fornecidos'
            }), 400
        
        print("DEBUG: Criando objeto ContactMessage")
        # Criar nova mensagem de contato
        # Converter newsletter para boolean (checkbox HTML envia 'on' quando marcado)
        newsletter_value = data.get('newsletter') == 'on'
        ...
        contact_message = ContactMessage(
            ...
            plano_escolhido=json.dumps(plano_data, ensure_ascii=False),
            data_envio=data.get('data_envio'),
            hora_envio=data.get('hora_envio'),
            timestamp=datetime.utcnow()
        )
        
        print("DEBUG: Adicionando mensagem ao banco de dados")
        db.session.add(contact_message)
        print("DEBUG: Commitando transação")
        db.session.commit()
        
        print(f"DEBUG: Nova mensagem de contato recebida de {data['nome']} ({data['email']}) para o plano {plano_data.get('nome', 'N/A')}")
...
# --- ROTAS PARA O DASHBOARD DE MENSAGENS DE CONTATO ---

@app.route('/admin/contact-messages')
@super_admin_required
def admin_contact_messages():
    """Página do dashboard para gerenciar mensagens de contato"""
    return render_template('admin/contact_messages.html')

@app.route('/api/admin/contact-messages')
@super_admin_required
def api_admin_contact_messages():
    """API para buscar todas as mensagens de contato"""
    try:
        print("DEBUG: Buscando mensagens de contato...")
        messages = ContactMessage.query.order_by(ContactMessage.created_at.desc()).all()
        print(f"DEBUG: Encontradas {len(messages)} mensagens")
        
        # Verificar se há mensagens e mostrar detalhes
        if messages:
            for i, msg in enumerate(messages[:3]):  # Mostrar apenas as 3 primeiras para debug
                print(f"DEBUG: Mensagem {i+1}: ID={msg.id}, Nome={msg.nome}, Email={msg.email}")
                print(f"DEBUG: Plano escolhido (raw): {msg.plano_escolhido}")
                print(f"DEBUG: Tipo do plano: {type(msg.plano_escolhido)}")
                print(f"DEBUG: Status: {msg.status}")
                print(f"DEBUG: Data criação: {msg.created_at}")
        else:
            print("DEBUG: Nenhuma mensagem encontrada no banco de dados")
            
        # Verificar se a tabela existe e tem dados
        try:
            from sqlalchemy import text
            result = db.session.execute(text("SELECT COUNT(*) FROM contact_message"))
            count = result.scalar()
            print(f"DEBUG: Total de registros na tabela contact_message: {count}")
        except Exception as e:
        ...
        for msg in messages:
            try:
                print(f"DEBUG: Processando mensagem {msg.id}: {msg.nome}")
                
                # Tentar fazer parse do JSON do plano escolhido
                plano_escolhido = None
                if msg.plano_escolhido:
                    try:
                        if isinstance(msg.plano_escolhido, str):
                            plano_escolhido = json.loads(msg.plano_escolhido)
                        else:
                            plano_escolhido = str(msg.plano_escolhido)
                    except (json.JSONDecodeError, TypeError) as json_error:
                        print(f"DEBUG: Erro JSON na mensagem {msg.id}: {json_error}")
                        plano_escolhido = str(msg.plano_escolhido)
                
                # Criar objeto de dados da mensagem
                message_obj = {
                    'id': msg.id,
                    'nome': str(msg.nome) if msg.nome else '',
                    'email': str(msg.email) if msg.email else '',
                    'telefone': str(msg.telefone) if msg.telefone else '',
                    'empresa': str(msg.empresa) if msg.empresa else '',
                    'cargo': str(msg.cargo) if msg.cargo else '',
                    'setor': str(msg.setor) if msg.setor else '',
                    'funcionarios': str(msg.funcionarios) if msg.funcionarios else '',
                    'urgencia': str(msg.urgencia) if msg.urgencia else '',
                    'mensagem': str(msg.mensagem) if msg.mensagem else '',
                    'newsletter': bool(msg.newsletter) if msg.newsletter is not None else False,
                    'plano_escolhido': plano_escolhido,
                    'data_envio': str(msg.data_envio) if msg.data_envio else '',
                    'hora_envio': str(msg.hora_envio) if msg.hora_envio else '',
                    'status': str(msg.status) if msg.status else 'novo',
                    'observacoes_admin': str(msg.observacoes_admin) if msg.observacoes_admin else '',
                    'created_at': msg.created_at.isoformat() if msg.created_at else None
                }
                
                print(f"DEBUG: Mensagem {msg.id} processada com sucesso")
                messages_data.append(message_obj)
                
            except Exception as e:
                print(f"DEBUG: Erro ao processar mensagem {msg.id}: {e}")
                import traceback
                traceback.print_exc()
                # Incluir mensagem com dados básicos se houver erro
                messages_data.append({
                    'id': msg.id if hasattr(msg, 'id') else 'N/A',
                    'nome': getattr(msg, 'nome', 'Erro') or 'Erro',
                    'email': getattr(msg, 'email', 'erro@erro.com') or 'erro@erro.com',
...
@app.route('/api/admin/contact-messages/<int:message_id>')
@super_admin_required
def api_admin_contact_message_detail(message_id):
    """API para buscar detalhes de uma mensagem específica"""
    try:
        message = ContactMessage.query.get_or_404(message_id)
        
        # Tentar fazer parse do JSON do plano escolhido
        plano_escolhido = None
        if message.plano_escolhido:
            try:
                plano_escolhido = json.loads(message.plano_escolhido)
            except (json.JSONDecodeError, TypeError):
                # Se falhar, usar o valor como string
                plano_escolhido = str(message.plano_escolhido)
        ...
        message_data = {
            ...
            'cargo': message.cargo,
            'setor': message.setor,
            'funcionarios': message.funcionarios,
            'urgencia': message.urgencia,
            'mensagem': message.mensagem,
            'newsletter': message.newsletter,
            'plano_escolhido': plano_escolhido,
            'data_envio': message.data_envio,
            'hora_envio': message.hora_envio,
            'status': message.status,
            'observacoes_admin': message.observacoes_admin,
...
@super_admin_required
def api_admin_contact_message_status(message_id):
    """API para atualizar o status de uma mensagem"""
    try:
        data = request.get_json()
        ...
        # Se houver observações, salvá-las também
        if 'observacoes_admin' in data:
            message.observacoes_admin = data['observacoes_admin']
    ...
    try:
        ...
        default_configs = [
            {
                ...
                'category': 'base',
                'base_price': 25.0,
                'tier_1_price': 25.0,  # Primeiras 10 unidades: sem desconto
                'tier_2_price': 22.5,  # 10-50 unidades: 10% desconto
                'tier_3_price': 20.0,  # 50-200 unidades: 20% desconto
                'tier_4_price': 17.5,  # 200-500 unidades: 30% desconto
                'tier_5_price': 15.0,  # 500-1000 unidades: 40% desconto
                'tier_6_price': 12.5,  # 1000+ unidades: 50% desconto
                'tier_1_limit': 10,
                'tier_2_limit': 50,
                'tier_3_limit': 200,
                'tier_4_limit': 500,
            ...
            },
            {
                ...
                'category': 'base',
                'base_price': 5.0,
                'tier_1_price': 5.0,   # Primeiras 100 unidades: sem desconto
                'tier_2_price': 4.5,   # 100-500 unidades: 10% desconto
                'tier_3_price': 4.0,   # 500-2000 unidades: 20% desconto
                'tier_4_price': 3.5,   # 2000-5000 unidades: 30% desconto
                'tier_5_price': 3.0,   # 5000-10000 unidades: 40% desconto
                'tier_6_price': 2.5,   # 10000+ unidades: 50% desconto
                'tier_1_limit': 100,
                'tier_2_limit': 500,
                'tier_3_limit': 2000,
                'tier_4_limit': 5000,
            ...
            },
            {
                ...
                'category': 'base',
                'base_price': 15.0,
                'tier_1_price': 15.0,  # Primeiros 20 mapas: sem desconto
                'tier_2_price': 13.5,  # 20-100 mapas: 10% desconto
                'tier_3_price': 12.0,  # 100-500 mapas: 20% desconto
                'tier_4_price': 10.5,  # 500-1000 mapas: 30% desconto
                'tier_5_price': 9.0,   # 1000-5000 mapas: 40% desconto
                'tier_6_price': 7.5,   # 5000+ mapas: 50% desconto
                'tier_1_limit': 20,
                'tier_2_limit': 100,
                'tier_3_limit': 500,
                'tier_4_limit': 1000,
            ...
            },
            {
                ...
                'category': 'tier',
                'base_price': 0.002,
                'tier_1_price': 0.002,  # Primeiras 10k visualizações: sem desconto
                'tier_2_price': 0.0018, # 10k-100k: 10% desconto
                'tier_3_price': 0.0016, # 100k-1M: 20% desconto
                'tier_4_price': 0.0014, # 1M-5M: 30% desconto
                'tier_5_price': 0.0012, # 5M-10M: 40% desconto
                'tier_6_price': 0.001,  # 10M+: 50% desconto
                'tier_1_limit': 10000,
                'tier_2_limit': 100000,
                'tier_3_limit': 1000000,
                'tier_4_limit': 5000000,
                'tier_5_limit': 10000000,
                'tier_6_limit': 100000000,
                'weight_factor': 0.8
            },
            {
                'name': 'Funcionário',
                'category': 'base',
                'base_price': 10.0,
                'tier_1_price': 10.0,  # Primeiros 25 funcionários: sem desconto
                'tier_2_price': 9.0,   # 25-100 funcionários: 10% desconto
                'tier_3_price': 8.0,   # 100-500 funcionários: 20% desconto
                'tier_4_price': 7.0,   # 500-1000 funcionários: 30% desconto
                'tier_5_price': 6.0,   # 1000-5000 funcionários: 40% desconto
                'tier_6_price': 5.0,   # 5000+ funcionários: 50% desconto
                'tier_1_limit': 25,
                'tier_2_limit': 100,
                'tier_3_limit': 500,
                'tier_4_limit': 1000,
            ...
            },
            {
                ...
                'tier_1_price': 0.5,   # Primeiros 50 GB: sem desconto
                'tier_2_price': 0.45,  # 50-250 GB: 10% desconto
                'tier_3_price': 0.4,   # 250-500 GB: 20% desconto
                'tier_4_price': 0.35,  # 500-1000 GB: 30% desconto
                'tier_5_price': 0.3,   # 1000-5000 GB: 40% desconto
                'tier_6_price': 0.25,  # 5000+ GB: 50% desconto
                'tier_1_limit': 50,
                'tier_2_limit': 250,
                'tier_3_limit': 500,
                'tier_4_limit': 1000,
        ...
        for config_data in default_configs:
            ...
            if not existing:
                ...
                # Atualizar preços base e peso se necessário
                if existing.base_price != config_data['base_price']:
                    existing.base_price = config_data['base_price']
                    print(f"DEBUG: Atualizando preço base de {existing.name}: {existing.base_price}")
                
                if existing.weight_factor != config_data['weight_factor']:
                    existing.weight_factor = config_data['weight_factor']
                    print(f"DEBUG: Atualizando peso de {existing.name}: {existing.weight_factor}")
                
                # Atualizar todos os tiers se necessário
                for i in range(1, 7):
                    tier_price_key = f'tier_{i}_price'
                    tier_limit_key = f'tier_{i}_limit'
                    
                    if tier_price_key in config_data:
                        current_price = getattr(existing, tier_price_key, 0)
                        new_price = config_data[tier_price_key]
                        if current_price != new_price:
                            setattr(existing, tier_price_key, new_price)
                            print(f"DEBUG: Atualizando {tier_price_key} de {existing.name}: {current_price} -> {new_price}")
                    
                    if tier_limit_key in config_data:
                        current_limit = getattr(existing, tier_limit_key, 0)
                        new_limit = config_data[tier_limit_key]
                        if current_limit != new_limit:
                            setattr(existing, tier_limit_key, new_limit)
                            print(f"DEBUG: Atualizando {tier_limit_key} de {existing.name}: {current_limit} -> {new_limit}")
        ...
        # Se não há configurações ativas, forçar ativação
        if not final_configs:
            print("DEBUG: Nenhuma configuração ativa encontrada, forçando ativação...")
            all_configs = PricingConfig.query.all()
            for config in all_configs:
                config.is_active = True
                print(f"DEBUG: Forçando ativação de: {config.name}")
            db.session.commit()
            print("DEBUG: Todas as configurações foram ativadas!")
...
def get_plan_prices():
    """Retorna os preços dos planos calculados dinamicamente com detalhes dos descontos por tier"""
    try:
        # Configurações dos planos (exatamente como na calculadora)
        plans = {
            'starter': {
                'projects': 5,
                'layers': 50,
        ...
        for plan_name, config in plans.items():
            # Calcular preço para cada plano usando a mesma função simplificada da calculadora
            configs = PricingConfig.query.filter_by(is_active=True).all()
            config_map = {}
            for cfg in configs:
            ...
            # Calcular usando a mesma lógica da calculadora
            if config.get('projects', 0) > 0 and 'Projeto' in config_map:
                cfg = config_map['Projeto']
                price = calculate_simple_tier_price(config['projects'], cfg)
                breakdown['Projetos'] = price
                total_price += price
            
            if config.get('layers', 0) > 0 and 'Camada' in config_map:
                cfg = config_map['Camada']
                price = calculate_simple_tier_price(config['layers'], cfg)
                breakdown['Camadas'] = price
                total_price += price
            
            if config.get('public_maps', 0) > 0 and 'Mapa Público' in config_map:
                cfg = config_map['Mapa Público']
                price = calculate_simple_tier_price(config['public_maps'], cfg)
                breakdown['Mapas Públicos'] = price
                total_price += price
            
            if config.get('page_views', 0) > 0 and 'Visualização' in config_map:
                cfg = config_map['Visualização']
                price = calculate_simple_tier_price(config['page_views'], cfg)
                breakdown['Visualizações'] = price
                total_price += price
            
            if config.get('employees', 0) > 0 and 'Funcionário' in config_map:
                cfg = config_map['Funcionário']
                price = calculate_simple_tier_price(config['employees'], cfg)
                breakdown['Funcionários'] = price
                total_price += price
            
            if config.get('server_usage', 0) > 0 and 'Servidor' in config_map:
                cfg = config_map['Servidor']
                price = calculate_simple_tier_price(config['server_usage'], cfg)
                breakdown['Servidor'] = price
                total_price += price
        ...
        print(f"Erro na API de planos: {e}")
        import traceback
        traceback.print_exc()
        return jsonify({
            'success': False,
...
def get_pricing_config():
    """Retorna as configurações de preços para uso na calculadora"""
    try:
        # Obter configurações de preços ativas
        configs = PricingConfig.query.filter_by(is_active=True).all()
        ...
        for config in configs:
            config_data.append({
                'id': config.id,
                'nome': config.name,
                'ativo': config.is_active,
                'preco_base': float(config.base_price),
                'tier_1_limit': int(config.tier_1_limit or 0),
                'tier_1_price': float(config.tier_1_price or 0),
                'tier_2_limit': int(config.tier_2_limit or 0),
                'tier_2_price': float(config.tier_2_price or 0),
                'tier_3_limit': int(config.tier_3_limit or 0),
                'tier_3_price': float(config.tier_3_price or 0),
                'tier_4_limit': 0,  # Não implementado no modelo atual
                'tier_4_price': 0,  # Não implementado no modelo atual
                'tier_5_limit': 0,  # Não implementado no modelo atual
                'tier_5_price': 0,  # Não implementado no modelo atual
                'tier_6_limit': 0,  # Não implementado no modelo atual
                'tier_6_price': 0   # Não implementado no modelo atual
            })
        
        return jsonify(config_data)
    except Exception as e:
        print(f"Erro ao obter configurações de preços: {e}")
        return jsonify({'error': 'Erro interno do servidor'}), 500

@app.route('/api/pricing/calculate', methods=['POST'])
def calculate_pricing_simple():
    """Endpoint para calcular preços usando a mesma lógica da calculadora JavaScript"""
    try:
        data = request.get_json()
        ...
        if not configs:
            return jsonify({'success': False, 'error': 'Nenhuma configuração de preços encontrada'}), 400
        
        # Mapear configurações por nome
        config_map = {}
        for config in configs:
        ...
        # Calcular preço para cada item usando a mesma lógica do JavaScript
        if data.get('projects', 0) > 0 and 'Projeto' in config_map:
            config = config_map['Projeto']
            price = calculate_simple_tier_price(data['projects'], config)
            breakdown['Projetos'] = price
            total_price += price
        
        if data.get('layers', 0) > 0 and 'Camada' in config_map:
            config = config_map['Camada']
            price = calculate_simple_tier_price(data['layers'], config)
            breakdown['Camadas'] = price
            total_price += price
        
        if data.get('public_maps', 0) > 0 and 'Mapa Público' in config_map:
            config = config_map['Mapa Público']
            price = calculate_simple_tier_price(data['public_maps'], config)
            breakdown['Mapas Públicos'] = price
            total_price += price
        
        if data.get('page_views', 0) > 0 and 'Visualização' in config_map:
            config = config_map['Visualização']
            price = calculate_simple_tier_price(data['page_views'], config)
            breakdown['Visualizações'] = price
            total_price += price
        
        if data.get('employees', 0) > 0 and 'Funcionário' in config_map:
            config = config_map['Funcionário']
            price = calculate_simple_tier_price(data['employees'], config)
            breakdown['Funcionários'] = price
            total_price += price
        
        if data.get('server_usage', 0) > 0 and 'Servidor' in config_map:
            config = config_map['Servidor']
            price = calculate_simple_tier_price(data['server_usage'], config)
            breakdown['Servidor'] = price
            total_price += price
        
        # Determinar plano recomendado
        recommended_plan = 'Personalizado'
        if (data.get('projects', 0) <= 5 and data.get('layers', 0) <= 50 and 
            data.get('page_views', 0) <= 10000):
            recommended_plan = 'Starter'
        elif (data.get('projects', 0) <= 25 and data.get('layers', 0) <= 250 and 
              data.get('page_views', 0) <= 100000):
            recommended_plan = 'Professional'
        elif (data.get('projects', 0) <= 150 and data.get('layers', 0) <= 1000 and 
              data.get('page_views', 0) <= 500000):
        ...
        print(f"Erro no cálculo de preços: {e}")
        import traceback
        traceback.print_exc()
        return jsonify({'success': False, 'error': 'Erro interno do servidor'}), 500

def calculate_simple_tier_price(quantity, config):
    """Função simplificada para calcular preço baseado no tier (mesma lógica do JavaScript)"""
    price = config.base_price
    
    # Aplicar desconto por tier (apenas tiers 1, 2 e 3 disponíveis)
    if quantity > (config.tier_3_limit or 0) and (config.tier_3_price or 0) > 0:
        price = config.tier_3_price
    elif quantity > (config.tier_2_limit or 0) and (config.tier_2_price or 0) > 0:
        price = config.tier_2_price
    elif quantity > (config.tier_1_limit or 0) and (config.tier_1_price or 0) > 0:
        price = config.tier_1_price
    
    return price * quantity

def calculate_pricing_internal_detailed(data):
    """Função interna para calcular preços com detalhes dos descontos por tier"""
    try:
        # Buscar configurações de preços ativas
        configs = PricingConfig.query.filter_by(is_active=True).all()
        ...
        total_cost = 0
        total_cost_without_tiers = 0  # Custo total sem aplicação de tiers
        breakdown = {}
        tier_breakdown = {}
        total_tier_discount = 0
        
        # Mapear configurações por categoria
        config_map = {}
        for config in configs:
            if config.name == 'Projeto':
                config_map['projects'] = config
            elif config.name == 'Camada':
                config_map['layers'] = config
            elif config.name == 'Mapa Público':
                config_map['public_maps'] = config
            elif config.name == 'Visualização':
                config_map['page_views'] = config
            elif config.name == 'Funcionário':
                config_map['employees'] = config
            elif config.name == 'Servidor':
                config_map['server_usage'] = config
        
        # Calcular custo para cada item
        if 'projects' in config_map:
            config = config_map['projects']
            result = calculate_item_cost_detailed(data.get('projects', 0), config)
            
            # Custo com tiers
            cost_with_tiers = result['cost'] * config.weight_factor
            # Custo sem tiers (usando preço base)
            cost_without_tiers = data.get('projects', 0) * config.base_price * config.weight_factor
            
            breakdown['projects'] = {
                'cost': result['cost'],
                'weight': config.weight_factor,
                'weighted_cost': cost_with_tiers,
                'cost_without_tiers': cost_without_tiers,
                'tier_discount': result['tier_discount'],
                'tier_details': result['tier_details']
            }
            total_cost += cost_with_tiers
            total_cost_without_tiers += cost_without_tiers
            total_tier_discount += result['tier_discount'] * config.weight_factor
            tier_breakdown['projects'] = result['tier_details']
            
        if 'layers' in config_map:
            config = config_map['layers']
            result = calculate_item_cost_detailed(data.get('layers', 0), config)
            
            cost_with_tiers = result['cost'] * config.weight_factor
            cost_without_tiers = data.get('layers', 0) * config.base_price * config.weight_factor
            ...
            total_cost_without_tiers += cost_without_tiers
            total_tier_discount += result['tier_discount'] * config.weight_factor
            tier_breakdown['layers'] = result['tier_details']
            
        if 'public_maps' in config_map:
            config = config_map['public_maps']
            result = calculate_item_cost_detailed(data.get('public_maps', 0), config)
            
            cost_with_tiers = result['cost'] * config.weight_factor
            cost_without_tiers = data.get('public_maps', 0) * config.base_price * config.weight_factor
            
            breakdown['public_maps'] = {
                'cost': result['cost'],
                'weight': config.weight_factor,
                'weighted_cost': cost_with_tiers,
                'cost_without_tiers': cost_without_tiers,
            ...
            total_cost_without_tiers += cost_without_tiers
            total_tier_discount += result['tier_discount'] * config.weight_factor
            tier_breakdown['public_maps'] = result['tier_details']
            
        if 'page_views' in config_map:
            config = config_map['page_views']
            result = calculate_item_cost_detailed(data.get('page_views', 0), config)
            
            cost_with_tiers = result['cost'] * config.weight_factor
            cost_without_tiers = data.get('page_views', 0) * config.base_price * config.weight_factor
            
            breakdown['page_views'] = {
                'cost': result['cost'],
                'weight': config.weight_factor,
                'weighted_cost': cost_with_tiers,
                'cost_without_tiers': cost_without_tiers,
            ...
            total_cost_without_tiers += cost_without_tiers
            total_tier_discount += result['tier_discount'] * config.weight_factor
            tier_breakdown['page_views'] = result['tier_details']
            
        if 'employees' in config_map:
            config = config_map['employees']
            result = calculate_item_cost_detailed(data.get('employees', 0), config)
            
            cost_with_tiers = result['cost'] * config.weight_factor
            cost_without_tiers = data.get('employees', 0) * config.base_price * config.weight_factor
            ...
            total_cost_without_tiers += cost_without_tiers
            total_tier_discount += result['tier_discount'] * config.weight_factor
            tier_breakdown['employees'] = result['tier_details']
            
        if 'server_usage' in config_map:
            config = config_map['server_usage']
            result = calculate_item_cost_detailed(data.get('server_usage', 0), config)
            
            cost_with_tiers = result['cost'] * config.weight_factor
            cost_without_tiers = data.get('server_usage', 0) * config.base_price * config.weight_factor
            
            breakdown['server_usage'] = {
                'cost': result['cost'],
                'weight': config.weight_factor,
                'weighted_cost': cost_with_tiers,
                'cost_without_tiers': cost_without_tiers,
            ...
            total_cost_without_tiers += cost_without_tiers
            total_tier_discount += result['tier_discount'] * config.weight_factor
            tier_breakdown['server_usage'] = result['tier_details']
        
        # Calcular desconto total real (valor sem tiers - valor com tiers)
        real_discount = total_cost_without_tiers - total_cost
        discount_percentage = ((real_discount / total_cost_without_tiers) * 100) if total_cost_without_tiers > 0 else 0
        
        return {
            'total_cost': round(total_cost, 2),
            'total_cost_without_tiers': round(total_cost_without_tiers, 2),
            'breakdown': breakdown,
            'tier_breakdown': tier_breakdown,
            'total_tier_discount': round(total_tier_discount, 2),
        ...
        print(f"Erro no cálculo interno detalhado: {e}")
        import traceback
        traceback.print_exc()
        return None

def calculate_item_cost_detailed(quantity, config):
    """Calcula o custo de um item com detalhes dos descontos por tier"""
    try:
        if quantity <= 0:
            return {
                'cost': 0,
        ...
        # Buscar preços de tier
        tier_prices = []
        tier_limits = []
        
        for i in range(1, 7):  # 6 tiers
            tier_price = getattr(config, f'tier_{i}_price', 0)
            tier_limit = getattr(config, f'tier_{i}_limit', 0)
            
            if tier_price > 0 and tier_limit > 0:
                tier_prices.append(tier_price)
                tier_limits.append(tier_limit)
        
        if not tier_prices:
            # Usar preço base se não houver tiers
            base_cost = quantity * config.base_price
            return {
                'cost': base_cost,
                'tier_discount': 0,
                'tier_details': [{
        ...
        # Calcular custo com tiers e descontos
        cost = 0
        remaining_quantity = quantity
        tier_details = []
        total_tier_discount = 0
        
        # Aplicar tiers em ordem crescente de volume (maior volume = maior desconto)
        for i, limit in enumerate(tier_limits):
            if remaining_quantity <= 0:
                break
                
            if i == 0:
                # Primeiro tier (sem desconto) - aplicado às primeiras unidades
                tier_quantity = min(remaining_quantity, limit)
                tier_cost = tier_quantity * tier_prices[i]
                cost += tier_cost
                
                tier_details.append({
                    'tier': i + 1,
                    'quantity': tier_quantity,
                    'price_per_unit': tier_prices[i],
                    'total': tier_cost,
                    'discount': 0,
                    'discount_percentage': 0
                })
                ...
                # Tiers subsequentes (com desconto progressivo) - aplicado às unidades seguintes
                tier_quantity = min(remaining_quantity, limit - tier_limits[i-1])
                if tier_quantity > 0:
                    # Calcular desconto comparado ao preço base
                    base_cost_for_tier = tier_quantity * config.base_price
                    tier_cost = tier_quantity * tier_prices[i]
                    tier_discount = base_cost_for_tier - tier_cost
                    
                    cost += tier_cost
                    total_tier_discount += tier_discount
                    
                    discount_percentage = ((tier_discount / base_cost_for_tier) * 100) if base_cost_for_tier > 0 else 0
        ...
        # Se ainda houver quantidade, usar o último preço (maior desconto)
        if remaining_quantity > 0 and tier_prices:
            last_tier_price = tier_prices[-1]
            last_tier_cost = remaining_quantity * last_tier_price
            cost += last_tier_cost
            
            # Calcular desconto para quantidade restante
            base_cost_for_remaining = remaining_quantity * config.base_price
            remaining_discount = base_cost_for_remaining - last_tier_cost
            total_tier_discount += remaining_discount
            
            discount_percentage = ((remaining_discount / base_cost_for_remaining) * 100) if base_cost_for_remaining > 0 else 0
            
            tier_details.append({
                'tier': len(tier_prices),
                'quantity': remaining_quantity,
                'price_per_unit': last_tier_price,
                'total': last_tier_cost,
        ...
        return {
            'cost': cost,
            'tier_discount': total_tier_discount,
            'tier_details': tier_details
        }
        
    except Exception as e:
        print(f"Erro ao calcular custo detalhado do item: {e}")
        return {
            'cost': 0,
            'tier_discount': 0,
            'tier_details': []
...
with app.app_context():
    ...
    print("DEBUG: Criando todas as tabelas do banco de dados...")
    db.create_all()
    print("DEBUG: Tabelas criadas com sucesso!")
    
    # Verificar se a tabela ContactMessage foi criada
    try:
        from sqlalchemy import inspect
        inspector = inspect(db.engine)
        tables = inspector.get_table_names()
        print(f"DEBUG: Tabelas existentes no banco: {tables}")
        if 'contact_message' in tables:
            print("DEBUG: Tabela contact_message encontrada!")
        else:
            print("DEBUG: ERRO: Tabela contact_message NÃO encontrada!")
    except Exception as e:
...
@app.route('/geographic-report')
@login_required
def geographic_report():
    """Página principal do Relatório Geográfico"""
    return render_template('geographic_report/index.html')

@app.route('/geographic-report/templates')
@login_required
def geographic_report_templates():
    """Página de templates do Relatório Geográfico"""
    return render_template('geographic_report/templates.html')

@app.route('/geographic-report/create')
@login_required
def create_geographic_report():
    """Criar novo relatório geográfico"""
    return render_template('geographic_report/create.html')

@app.route('/geographic-report/edit/<int:report_id>')
@login_required
def edit_geographic_report(report_id):
    """Editar relatório geográfico existente"""
    return render_template('geographic_report/edit.html', report_id=report_id)

@app.route('/geographic-report/view/<int:report_id>')
def view_geographic_report(report_id):
    """Visualizar relatório geográfico (público)"""
    return render_template('geographic_report/view.html', report_id=report_id)

@app.route('/geographic-report/template/<template_name>')
def view_template(template_name):
    """Visualizar template específico"""
    template_path = f'geographic_report/templates/{template_name}.html'
    try:
        return render_template(template_path)
    except:
        abort(404)

@app.route('/api/geographic-report', methods=['POST'])
@login_required
def api_create_geographic_report():
    """API para criar relatório geográfico"""
    data = request.get_json()
    
    # Criar novo relatório
    new_report = GeographicReport(
...
@app.route('/api/geographic-report/<int:report_id>', methods=['PUT'])
@login_required
def api_update_geographic_report(report_id):
    """API para atualizar relatório geográfico"""
    data = request.get_json()
    
    report = GeographicReport.query.get_or_404(report_id)
...
@app.route('/api/geographic-report/<int:report_id>/publish', methods=['POST'])
@login_required
def api_publish_geographic_report(report_id):
    """API para publicar relatório geográfico"""
    data = request.get_json()
    ...
    report.config.update(data.get('design', {}))
    ...
    return jsonify({
        'success': True,
        'message': 'Relatório publicado com sucesso!',
        'public_url': url_for('view_geographic_report', report_id=report.id, _external=True)
    })
