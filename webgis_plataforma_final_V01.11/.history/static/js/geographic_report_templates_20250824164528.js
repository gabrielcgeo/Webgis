...
class TemplateManager {
    ...
    loadTemplates() {
        ...
        this.templates = {
            ...
            'corporate-report': {
                id: 'corporate-report',
                title: 'Relatório Corporativo Profissional',
                description: 'Template corporativo com estrutura hierárquica, navegação lateral e design profissional para relatórios empresariais.',
                category: 'story-focused',
                layout: 'classic',
                complexity: 'advanced',
                focus: 'story',
                elements: ['hierarchical-nav', 'chapters', 'subchapters', 'maps', 'media', 'documents', 'charts'],
                preview: 'https://via.placeholder.com/400x300/34495E/ffffff?text=Relatório+Corporativo',
                color: '#34495E',
                template: {
                    structure: 'hierarchical',
                    navigation: 'sidebar',
                    layout: 'professional',
                    features: ['editable-content', 'dynamic-maps', 'media-gallery', 'document-links', 'responsive-design']
                }
            },
            'events-map': {
                id: 'events-map',
    ...
    previewTemplate(templateId) {
        console.log('previewTemplate chamado com ID:', templateId);
        ...
        console.log('Elementos do modal:', { modal, title, content, useBtn });
...
document.addEventListener('DOMContentLoaded', function() {
    console.log('DOM carregado, criando TemplateManager...');
    
    try {
        templateManager = new TemplateManager();
        console.log('TemplateManager criado com sucesso:', templateManager);
        
        // Expor métodos globalmente para compatibilidade
        window.previewTemplate = (templateId) => {
            console.log('previewTemplate chamado globalmente com:', templateId);
            return templateManager.previewTemplate(templateId);
        };
        window.useTemplate = (templateId) => {
            console.log('useTemplate chamado globalmente com:', templateId);
            return templateManager.useTemplate(templateId);
        };
        ...
        window.testPreview = () => {
            ...
            console.log('TemplateManager disponível:', templateManager);
            console.log('Bootstrap disponível:', typeof bootstrap);
            console.log('Modal disponível:', document.getElementById('templatePreviewModal'));
            
            // Testar preview do primeiro template
            if (templateManager && templateManager.templates) {
                const firstTemplateId = Object.keys(templateManager.templates)[0];
                console.log('Testando preview do template:', firstTemplateId);
                templateManager.previewTemplate(firstTemplateId);
            } else {
                console.error('TemplateManager não está disponível');
            }
        };
        
        // Função para testar modal simples
        window.testSimpleModal = () => {
            console.log('Testando modal simples...');
            const testModal = document.getElementById('testModal');
            if (testModal) {
                try {
                    const bootstrapModal = new bootstrap.Modal(testModal);
                    bootstrapModal.show();
                ...
                console.error('Modal de teste não encontrado!');
            }
        };
        
        console.log('Métodos expostos globalmente:', {
            previewTemplate: window.previewTemplate,
            useTemplate: window.useTemplate,
            applyTemplateFilters: window.applyTemplateFilters,
            testPreview: window.testPreview,
            testSimpleModal: window.testSimpleModal
        });
    } catch (error) {
        console.error('Erro ao criar TemplateManager:', error);
    }
});
