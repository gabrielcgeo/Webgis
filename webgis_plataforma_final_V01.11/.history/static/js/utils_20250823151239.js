...
/**
 * Formata um valor de armazenamento com a unidade mais apropriada
 * Converte automaticamente entre GB, TB, PB conforme necessário
 * @param {number} valor - Valor em GB
 * @param {number} casasDecimais - Número de casas decimais (padrão: 1)
 * @returns {string} Valor formatado com a unidade mais apropriada
 */
function formatarArmazenamentoInteligente(valor, casasDecimais = 1) {
    if (valor === null || valor === undefined || isNaN(valor)) {
        return '0 GB';
    }
    ...
    // Se o valor for >= 1.000.000 GB (1 PB), converter para PB
    if (numero >= 1000000) {
        const valorPB = numero / 1000000;
        const formatado = valorPB.toLocaleString('pt-BR', {
            minimumFractionDigits: casasDecimais,
            maximumFractionDigits: casasDecimais
        });
        return `${formatado} PB`;
    ...
    // Se o valor for >= 1000 GB (1 TB), converter para TB
    if (numero >= 1000) {
        const valorTB = numero / 1000;
        const formatado = valorTB.toLocaleString('pt-BR', {
            minimumFractionDigits: casasDecimais,
            maximumFractionDigits: casasDecimais
        });
        return `${formatado} TB`;
    ...
    // Para valores < 1000 GB, manter em GB
    const formatado = numero.toLocaleString('pt-BR', {
        minimumFractionDigits: casasDecimais,
        maximumFractionDigits: casasDecimais
    });
 ...
 * Converte automaticamente para TB quando o valor é >= 1000 GB
 * @param {number} valor - Valor em GB
 * @param {number} casasDecimais - Número de casas decimais (padrão: 1)
 * @returns {string} Valor formatado com separadores brasileiros
 */
function formatarArmazenamento(valor, casasDecimais = 1) {
    ...
    // Se o valor for >= 1000 GB, converter para TB
    if (numero >= 1000) {
        const valorTB = numero / 1000;
        const formatado = valorTB.toLocaleString('pt-BR', {
            minimumFractionDigits: casasDecimais,
...
window.formatarArmazenamentoInteligente = formatarArmazenamentoInteligente;
