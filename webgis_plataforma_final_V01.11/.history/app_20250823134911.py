...
def calculate_pricing_internal_detailed(data):
    ...
    try:
        print(f"Calculando preços para dados: {data}")
        ...
        if not configs:
            print("Nenhuma configuração de preços encontrada")
            return None
        ...
        print(f"Configurações mapeadas: {list(config_map.keys())}")
        
        # Calcular custo para cada item
        if 'projects' in config_map:
            config = config_map['projects']
            print(f"Processando projects: {config.name}, base_price: {config.base_price}")
            result = calculate_item_cost_detailed(data.get('projects', 0), config)
            print(f"Resultado projects: {result}")
            breakdown['projects'] = {
                'cost': result['cost'],
                'weight': config.weight_factor,
                'weighted_cost': result['cost'] * config.weight_factor,
                'tier_discount': result['tier_discount'],
                'tier_details': result['tier_details']
            }
            total_cost += result['cost'] * config.weight_factor
        ...
        if 'layers' in config_map:
            ...
            print(f"Processando layers: {config.name}, base_price: {config.base_price}")
            result = calculate_item_cost_detailed(data.get('layers', 0), config)
            print(f"Resultado layers: {result}")
            breakdown['layers'] = {
                'cost': result['cost'],
                'weight': config.weight_factor,
                'weighted_cost': result['cost'] * config.weight_factor,
        ...
        if 'public_maps' in config_map:
            ...
            print(f"Processando public_maps: {config.name}, base_price: {config.base_price}")
            result = calculate_item_cost_detailed(data.get('public_maps', 0), config)
            print(f"Resultado public_maps: {result}")
        ...
        if 'page_views' in config_map:
            ...
            print(f"Processando page_views: {config.name}, base_price: {config.base_price}")
            result = calculate_item_cost_detailed(data.get('page_views', 0), config)
            print(f"Resultado page_views: {result}")
        ...
        if 'employees' in config_map:
            ...
            print(f"Processando employees: {config.name}, base_price: {config.base_price}")
            result = calculate_item_cost_detailed(data.get('employees', 0), config)
            print(f"Resultado employees: {result}")
            breakdown['employees'] = {
                'cost': result['cost'],
                'weight': config.weight_factor,
                'weighted_cost': result['cost'] * config.weight_factor,
        ...
        if 'server_usage' in config_map:
            ...
            print(f"Processando server_usage: {config.name}, base_price: {config.base_price}")
            result = calculate_item_cost_detailed(data.get('server_usage', 0), config)
            print(f"Resultado server_usage: {result}")
        ...
        print(f"Total cost: {total_cost}, Total tier discount: {total_tier_discount}")
