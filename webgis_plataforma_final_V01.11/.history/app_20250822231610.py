...
def process_qml_xml_manual(filepath):
    ...
    usando abordagem de busca direta por cores
    """
    try:
        import xml.etree.ElementTree as ET
        
        print(f"QML Debug: Processando arquivo: {filepath}")
        ...
        print(f"QML Debug: Tipo de renderer encontrado: {renderer_type}")
        
        if renderer_type == 'categorizedSymbol':
            return extract_xml_categorized_style_direct(renderer)
        elif renderer_type == 'graduatedSymbol':
            return extract_xml_graduated_style_direct(renderer)
        elif renderer_type == 'singleSymbol':
            return extract_xml_single_style_direct(renderer)
        else:
            return {'success': False, 'error': f'Tipo de renderer não suportado: {renderer_type}'}
            
    except Exception as e:
        print(f"QML Debug: Erro no parser XML: {str(e)}")
        return {'success': False, 'error': f'Erro no parser XML manual: {str(e)}'}


def extract_xml_categorized_style_direct(renderer):
    """Extrai estilo categorizado usando busca direta por cores"""
    try:
        print("QML Debug: Extraindo categorias com busca direta")
        categories = []
        
        # Buscar todas as categorias
        category_elements = renderer.findall('.//category')
        print(f"QML Debug: Encontradas {len(category_elements)} categorias")
        
        for i, category in enumerate(category_elements):
            value = category.get('value', '')
            label = category.get('label', value)
            
            print(f"QML Debug: Processando categoria {i}: value='{value}', label='{label}'")
            
            # Busca direta por cores em todo o conteúdo da categoria
            color = extract_color_from_element_direct(category)
            
            print(f"QML Debug: Cor extraída para categoria {i}: {color}")
        ...
        return {'success': False, 'error': f'Erro ao extrair categorias: {str(e)}'}


def extract_color_from_element_direct(element):
    """
    Extrai cor de um elemento XML usando busca direta
    """
    try:
        # Converter elemento para string para busca por regex
        import re
        
        element_str = ET.tostring(element, encoding='unicode')
        print(f"QML Debug: Conteúdo do elemento: {element_str[:200]}...")
        
        # Buscar por padrões de cor
        color_patterns = [
            r'k="color"[^>]*v="([^"]*)"',  # prop k="color" v="..."
            r'k="fill_color"[^>]*v="([^"]*)"',  # prop k="fill_color" v="..."
            r'k="fillColor"[^>]*v="([^"]*)"',  # prop k="fillColor" v="..."
            r'k="fill"[^>]*v="([^"]*)"',  # prop k="fill" v="..."
            r'color="([^"]*)"',  # atributo color="..."
            r'fill="([^"]*)"',  # atributo fill="..."
        ]
        
        for pattern in color_patterns:
            matches = re.findall(pattern, element_str)
            if matches:
                color = matches[0]
                if is_valid_color(color):
                    print(f"QML Debug: Cor encontrada com padrão {pattern}: {color}")
                    return color
        
        # Busca por valores que parecem cores
        color_values = re.findall(r'v="([^"]*)"', element_str)
        for value in color_values:
            if is_valid_color(value):
                print(f"QML Debug: Cor encontrada em valor: {value}")
                return value
        
        # Busca por cores em atributos
        for key, value in element.attrib.items():
            if 'color' in key.lower() and is_valid_color(value):
                print(f"QML Debug: Cor encontrada em atributo {key}: {value}")
                return value
        
        print(f"QML Debug: Nenhuma cor válida encontrada, usando padrão")
        return '#cccccc'
        
    except Exception as e:
        print(f"QML Debug: Erro ao extrair cor: {str(e)}")
        return '#cccccc'


def is_valid_color(color_str):
    """
    Verifica se uma string representa uma cor válida
    """
    if not color_str:
        return False
    ...
    color_names = [
        ...
        'white', 'black', 'orange', 'purple', 'brown', 'pink', 
        'gray', 'grey', 'navy', 'teal', 'lime', 'olive', 'maroon'
    ]
    
    return color_str.lower() in color_names
...
def extract_xml_graduated_style_direct(renderer):
    """Extrai estilo graduado usando busca direta por cores"""
    try:
        print("QML Debug: Extraindo ranges com busca direta")
        ranges = []
        
        # Buscar todos os ranges
        range_elements = renderer.findall('.//range')
        print(f"QML Debug: Encontrados {len(range_elements)} ranges")
        
        for i, range_elem in enumerate(range_elements):
            lower = float(range_elem.get('lower', 0))
            upper = float(range_elem.get('upper', 0))
            label = range_elem.get('label', f'{lower} - {upper}')
            
            print(f"QML Debug: Processando range {i}: {lower} - {upper}, label='{label}'")
            
            # Busca direta por cores
            color = extract_color_from_element_direct(range_elem)
            
            print(f"QML Debug: Cor extraída para range {i}: {color}")
        ...
        print(f"QML Debug: Erro ao extrair ranges: {str(e)}")
        return {'success': False, 'error': f'Erro ao extrair ranges do XML: {str(e)}'}


def extract_xml_single_style_direct(renderer):
    """Extrai estilo único usando busca direta por cores"""
    try:
        print("QML Debug: Extraindo estilo único com busca direta")
        
        # Busca direta por cores
        fill_color = extract_color_from_element_direct(renderer)
        
        # Para estilo único, tentar extrair cor da borda também
        stroke_color = '#333333'
        stroke_width = 1
        
        # Procurar por propriedades específicas de borda
        for prop in renderer.iter('prop'):
            key = prop.get('k', '')
            value = prop.get('v', '')
            
            if 'outline' in key.lower() or 'stroke' in key.lower():
                if 'color' in key.lower() and is_valid_color(value):
                    stroke_color = value
        ...
        print(f"QML Debug: Cores extraídas - fill: {fill_color}, stroke: {stroke_color}, width: {stroke_width}")
        ...
        print(f"QML Debug: Erro ao extrair estilo único: {str(e)}")
        return {'success': False, 'error': f'Erro ao extrair estilo único do XML: {str(e)}'}


def extract_xml_graduated_style(renderer):
    """Extrai estilo graduado do XML (mantido para compatibilidade)"""
    return extract_xml_graduated_style_direct(renderer)


def extract_xml_single_style(renderer):
    """Extrai estilo único do XML (mantido para compatibilidade)"""
    return extract_xml_single_style_direct(renderer)
...
def process_arcgis_file(filepath, layer, file_ext):
    ...
    """
    try:
        print(f"ArcGIS Debug: Processando arquivo {file_ext}: {filepath}")
        
        # Verificar se o arquivo é binário ou XML
        with open(filepath, 'rb') as f:
            first_bytes = f.read(10)
        
        print(f"ArcGIS Debug: Primeiros bytes: {first_bytes}")
        ...
        # Para arquivos .stylx, tentar como SQLite
        if file_ext == 'stylx':
            return process_stylx_as_sqlite(filepath)
        ...
        print(f"ArcGIS Debug: Erro: {str(e)}")
        return {'success': False, 'error': f'Erro ao processar arquivo ArcGIS: {str(e)}'}


def process_stylx_as_sqlite(filepath):
    """
    Processa arquivo .stylx como banco SQLite
    """
    try:
        import sqlite3
        
        print(f"STYLX Debug: Processando como SQLite: {filepath}")
        ...
        print(f"STYLX Debug: Tabelas encontradas: {[t[0] for t in tables]}")
        
        # Procurar por tabelas relacionadas a símbolos
        symbol_tables = []
        for table in tables:
            table_name = table[0]
            if 'symbol' in table_name.lower() or 'style' in table_name.lower():
                symbol_tables.append(table_name)
        
        print(f"STYLX Debug: Tabelas de símbolos: {symbol_tables}")
        
        # Tentar extrair informações de símbolos
        styles = []
        
        for table in symbol_tables[:3]:  # Limitar a 3 tabelas para não sobrecarregar
            try:
                ...
                print(f"STYLX Debug: Tabela {table} - Colunas: {columns}")
                print(f"STYLX Debug: Tabela {table} - Primeiras linhas: {rows[:2]}")
                
                # Procurar por cores nas colunas
                for row in rows:
                    row_dict = dict(zip(columns, row))
                    
                    # Procurar por valores que parecem cores
                    for col_name, value in row_dict.items():
                        if value and isinstance(value, str):
                            if is_valid_color(value):
                                ...
                                print(f"STYLX Debug: Cor encontrada em {table}.{col_name}: {value}")
                
            except Exception as e:
                print(f"STYLX Debug: Erro ao processar tabela {table}: {str(e)}")
                continue
        ...
        if styles:
            return {
                'success': True,
                'styleType': 'single',  # Para .stylx, assumir estilo único por enquanto
                'fillColor': styles[0]['color'] if styles else '#38bdf8',
                'meta': {
            ...
            return {
                'success': False,
                'error': 'Nenhuma cor encontrada no arquivo .stylx',
                'meta': {
                    'tables': [t[0] for t in tables],
                    'symbol_tables': symbol_tables
                }
        ...
        print(f"STYLX Debug: Erro ao processar como SQLite: {str(e)}")
        return {'success': False, 'error': f'Erro ao processar arquivo .stylx: {str(e)}'}
