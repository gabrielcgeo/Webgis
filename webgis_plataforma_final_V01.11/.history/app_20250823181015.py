...
def get_pricing_config():
    ...
    try:
        ...
        configs = PricingConfig.query.filter_by(ativo=True).all()
        ...
        for config in configs:
            config_data.append({
                ...
                'nome': config.nome,
                'ativo': config.ativo,
                'preco_base': float(config.preco_base),
                'tier_1_limit': int(config.tier_1_limit),
                'tier_1_price': float(config.tier_1_price),
                'tier_2_limit': int(config.tier_2_limit),
                'tier_2_price': float(config.tier_2_price),
                'tier_3_limit': int(config.tier_3_limit),
                'tier_3_price': float(config.tier_3_price),
                'tier_4_limit': int(config.tier_4_limit),
                'tier_4_price': float(config.tier_4_price),
                'tier_5_limit': int(config.tier_5_limit),
                'tier_5_price': float(config.tier_5_price),
                'tier_6_limit': int(config.tier_6_limit),
                'tier_6_price': float(config.tier_6_price)
            })
