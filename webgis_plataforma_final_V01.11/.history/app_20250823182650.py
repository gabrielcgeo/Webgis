...
def get_plan_prices():
    ...
    try:
        ...
        for plan_name, config in plans.items():
            # Calcular preço para cada plano usando a mesma função da calculadora
            result = calculate_pricing_internal_detailed(config)
            if result:
                plan_prices[plan_name] = {
                    'original_price': result['total_cost_without_tiers'],
                    'final_price': result['total_cost'],
                    'discount': result['discount_percentage'],
                    'config': config,
                    'tier_breakdown': result['tier_breakdown'],
                    'total_tier_discount': result['total_tier_discount'],
                    'real_discount': result['real_discount']
                }
