...
def extract_xml_categorized_style(renderer):
    """Extrai estilo categorizado do XML"""
    try:
        categories = []
        ...
        for category in renderer.findall('.//category'):
            ...
            # Procurar cor em diferentes locais
            color = '#cccccc'
            
            # Procurar em propriedades do símbolo
...
def process_arcgis_file(filepath, layer, file_ext):
    ...
    """
    try:
        # Tentar usar ArcPy se disponível
        try:
            import arcpy
            
            # Para arquivos .lyr, tentar extrair informações
            if file_ext == 'lyr':
            ...
            # ArcPy não disponível, tentar como XML
            if file_ext == 'lyr':
                return process_lyr_as_xml(filepath)
            else:
                return {'success': False, 'error': f'ArcPy não disponível e processamento XML não implementado para .{file_ext}'}
...
def process_lyr_as_xml(filepath):
    ...
    try:
        ...
        # Implementação básica - pode precisar de ajustes baseado na estrutura real
        
        return {'success': False, 'error': 'Parser XML para arquivos .lyr ainda não implementado completamente'}
