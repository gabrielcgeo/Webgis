...
def run_cleanup():
    with app.app_context():
        ...
        for company in companies_to_delete:
            ...
            for layer in layers:
                # Lida tanto com arquivos soltos quanto com shapefiles em pastas
                if '/' in layer.filename or '\' in layer.filename:
                     dir_path = os.path.join(app.config['UPLOAD_FOLDER'], os.path.dirname(layer.filename))
                     if os.path.isdir(dir_path):
                        try:
                            shutil.rmtree(dir_path)
                            print(f"    - Removido diretório da camada: {dir_path}")
                        except OSError as e:
                            print(f"    - Erro ao remover diretório {dir_path}: {e}")
                else:
                    base_path = os.path.join(app.config['UPLOAD_FOLDER'], os.path.splitext(layer.filename)[0])
            ...
            ExternalService.query.filter_by(company_id=company.id).delete(synchronize_session=False)
