...
def upload_file_action():
    ...
    if filename.lower().endswith(('.zip', '.kmz')):
        try:
            zip_buffer = io.BytesIO(file.read())
            with zipfile.ZipFile(zip_buffer, 'r') as zip_ref:
                shp_files = [s for s in zip_ref.namelist() if s.lower().endswith(('.shp', '.kml'))]
                if not shp_files:
                    flash(f'Arquivo {filename.upper()} não contém um arquivo .shp ou .kml.', 'danger')
                    return redirect(url_for('upload_layer_page'))
                base_name = os.path.splitext(shp_files[0])[0]
                final_filename = f"{base_name}.shp" if main_files[0].lower().endswith('.shp') else f"{base_name}.kml"
                zip_ref.extractall(app.config['UPLOAD_FOLDER'])
        except Exception as e:
            flash(f"Erro ao processar arquivo ZIP/KMZ: {e}", 'danger')
            return redirect(url_for('upload_layer_page'))
    else:
